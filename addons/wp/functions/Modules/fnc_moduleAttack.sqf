#include "script_component.hpp"
/*
 * Author: bluefield-creator
 * Zeus module. Dropped on a group it attacks the nearest dynamic target or asks for one;
 * dropped on the ground it asks which group should attack the module position.
 *
 * Arguments:
 * Arma 3 Module Function Parameters
 *
 * Return Value:
 * NONE
 *
 * Public: No
*/
params [["_mode", "", [""]], ["_input", [], [[]]]];

switch (_mode) do {
    // Default object init
    case "init": {
        if (is3DEN) exitWith {};
        _input params [["_logic", objNull, [objNull]], ["_isActivated", true, [true]], ["_isCuratorPlaced", false, [true]]];
        if !(_isActivated && local _logic) exitWith {};
        if (_isCuratorPlaced) then {
            //--- Get group under cursor
            private _group = GET_CURATOR_GRP_UNDER_CURSOR;

            if (isNull _group) then {
                private _groups = allGroups select {!isPlayer (leader _x) && {((units _x) findIf {alive _x}) != -1}};
                _groups = [_groups, [], {_logic distance (leader _x)}, "ASCEND"] call BIS_fnc_sortBy;
                [LSTRING(Module_TaskAttack_DisplayName),
                    [
                        [LSTRING(Groups_DisplayName), "DROPDOWN", LSTRING(Groups_ToolTip), _groups apply {format ["%1 - %2 (%3 m)", side _x, groupId _x, round ((leader _x) distance _logic)]}, 0],
                        [LSTRING(Module_TaskAttack_Radius_DisplayName), "SLIDER", LSTRING(Module_TaskAttack_Radius_Tooltip), [10, 300], [2, 1], TASK_ATTACK_SIZE, 2]
                    ], {
                        params ["_data", "_args"];
                        _args params ["_groups", "_logic"];
                        _data params ["_groupIndex", "_radius"];
                        private _group = _groups select _groupIndex;
                        [QGVAR(taskAttack), [_group, getPos _logic, _radius], leader _group] call CBA_fnc_targetEvent;
                        deleteVehicle _logic;
                    }, {
                        params ["", "_logic"];
                        deleteVehicle _logic;
                    }, {
                        params ["", "_logic"];
                        deleteVehicle _logic;
                    }, [_groups, _logic]] call EFUNC(main,showDialog);
            } else {
                _logic setVehicleVarName LLSTRING(Self);
                private _targets = [_logic];
                GVAR(ModuleTargets) = GVAR(ModuleTargets) - [objNull];
                _targets append GVAR(ModuleTargets);
                _targets = [_targets, [], {_logic distance _x}, "ASCEND"] call BIS_fnc_sortBy;

                [LSTRING(Module_TaskAttack_DisplayName),
                    [
                        [LSTRING(Centers_DisplayName), "DROPDOWN", LSTRING(Centers_ToolTip), _targets apply {format ["%1 (%2 m)", vehicleVarName _x, round (_x distance _logic)]}, 0],
                        [LSTRING(Module_TaskAttack_Radius_DisplayName), "SLIDER", LSTRING(Module_TaskAttack_Radius_Tooltip), [10, 300], [2, 1], TASK_ATTACK_SIZE, 2]
                    ], {
                        params ["_data", "_args"];
                        _args params ["_group", "_logic", "_targets"];
                        _data params ["_targetIndex", "_radius"];
                        private _target = _targets select _targetIndex;
                        [QGVAR(taskAttack), [_group, getPos _target, _radius], leader _group] call CBA_fnc_targetEvent;
                        deleteVehicle _logic;
                    }, {
                        params ["", "_logic"];
                        deleteVehicle _logic;
                    }, {
                        params ["", "_logic"];
                        deleteVehicle _logic;
                    }, [_group, _logic, _targets]
                ] call EFUNC(main,showDialog);
            };
        } else {
            private _groups = synchronizedObjects _logic apply {group _x};
            _groups = _groups arrayIntersect _groups;
            private _area = _logic getVariable ["objectarea", [TASK_ATTACK_SIZE, TASK_ATTACK_SIZE, 0, false, -1]];
            private _radius = _area select ((_area select 0) < (_area select 1));
            {
                [QGVAR(taskAttack), [_x, getPos _logic, _radius], leader _x] call CBA_fnc_targetEvent;
            } forEach _groups;
            deleteVehicle _logic;
        };
    };
};
